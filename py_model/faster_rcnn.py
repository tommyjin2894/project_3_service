...
def rcnn_out(image: Image.Image):
    ...
    logger.info(f"Detected Labels: {[int(i) for i in Detected]}, {detected_to_list}")
...
if __name__ == "__main__":
    ...
    # 결과 이미지를 저장
    # im from array
    test_image_result_array = Image.fromarray(rcnn_out(test_image)[0])
    test_image_result_array.save(f"result/rcnn/{time_}.jpg")